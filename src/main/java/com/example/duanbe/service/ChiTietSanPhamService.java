...
//                    chiTietSanPham.setTrang_thai(chiTietSanPhamRequest.getTrang_thai());
    ...
    public ResponseEntity<List<SanPhamView>> timKiemVaLoc(
        ...
        if (giaBanMin != null) {
            finalStream = finalStream.filter(ctsp -> ctsp.getGia_ban() != null && ctsp.getGia_ban().floatValue() >= giaBanMin);
        }

        if (giaBanMax != null) {
            finalStream = finalStream.filter(ctsp -> ctsp.getGia_ban() != null && ctsp.getGia_ban().floatValue() <= giaBanMax);
        }
        ...
        if (!isEmpty(listThuongHieu)) {
            finalStream = finalStream.filter(ctsp -> {
                ...
                return listThuongHieu.stream().anyMatch(th -> th != null && th.trim().equalsIgnoreCase(thuongHieuValue));
            });
        }
